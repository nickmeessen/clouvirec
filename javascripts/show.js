...
var Show = function(id, data) {
    ...
    var untrackLink = $('<a>').html("Untrack").click(function() {
        new Messi('Are you sure you want to untrack this show?<br />You won\t be notified about this show again.', {
            title: 'Untrack Show',
    ...
    var showPoster = $('<div>').html($('<img>').attr('src', "images/showLoading.png")).addClass('showLoading').addClass('showPoster');
    ...
    dlButton.src = "images/download.png";
    reqButton.src = "images/request.png";

    var showLoading1 = new Image();
    var showLoading2 = new Image();

    showLoading1.src = "images/showLoading.png";

    showLoading1.onload = function() {
        showLoading2.src = "images/loader2.gif";
    ...
    function fetchSeason(season) {
        ...
        $.get("episodes/" + tvdbid + "/" + season + ".json",
            ...
            function(data) {
                ...
                for (var ep in data) {
                    ...
                    if (data[ep].status == "Avaliable") {
                        button = $('<a>').append($('<img src="images/download.png">').attr('id', data[ep].id)).attr('href', "/episodes/" + tvdbid + "/" + data[ep].season + "/" + data[ep].number);
                        ...
                        button = $('<a>').append($('<img src="images/request.png">').attr('id', data[ep].id)).attr('href', "/episodes/" + tvdbid + "/" + data[ep].season + "/" + data[ep].number + "/request").click(requestEpisode);
                ...
                if (ss !== 0) {
                    ...
                    posterImage.src = "shows/" + tvdbid + "/poster.jpg";
                    posterImage.onload = function() {

                        preloaded = true;
                        showPoster.replaceWith($('<div>').html($('<img>').attr('src', "shows/" + tvdbid + "/poster.jpg").addClass('showPoster')).click(expandShow));
    ...
    function requestEpisode(event) {
        ...
        if (target.nodeName === "IMG") {
            ...
            url = $(target.parentNode).attr('href');
        ...
        new Messi('Episode is requested! You will be notified when it\'s avaliable :)', {
            modal: true,
            titleClass: 'info',
            buttons: [{
                id: 0,
